...
class WeatherService {
  ...
  static mapIcon(code) {
    ...
    if (
      [
        176, 263, 281, 296, 302, 308, 311, 314, 317, 320, 323, 326, 329, 332, 335, 338, 350, 353,
        356, 359, 362, 365, 374, 377,
      ].includes(c)
    )
      return '10d'; // Rain
  ...
  static buildAlerts(temp, humidity, precip, wind) {
    ...
    if (temp > 38)
      alerts.push({
        severity: 'danger',
        message: `Extreme heat (${Math.round(temp)}°C). Water crops early morning. Avoid midday spraying.`,
      });
    if (precip > 10)
      alerts.push({
        severity: 'danger',
        message: 'Heavy rain detected. Harvest sensitive crops immediately and cover storage.',
      });
    if (humidity > 82)
      alerts.push({
        severity: 'warning',
        message: `High humidity (${Math.round(humidity)}%). Risk of fungal disease. Monitor crops closely.`,
      });
    if (wind > 40)
      alerts.push({
        severity: 'warning',
        message: `Strong winds (${Math.round(wind)} km/h). Secure greenhouse covers and delay spraying.`,
      });
    return alerts;
  }
  ...
  static pestTip(humidity, temp) {
    if (humidity > 80 && temp > 25)
      return 'High risk of fungal growth and aphid infestation. Apply preventive fungicide.';
