  ...
  try {
    ...
    if (location) {
      try {
        ...
        // Fallback if location parsing fails (e.g. from FormData)
      }
      ...
      updateData.location = {
        type: 'Point',
        coordinates: [lng, lat],
        address: req.body.address || req.user.location?.address,
        city: req.body.city || req.user.location?.city,
        state: req.body.state || req.user.location?.state,
        pincode: req.body.pincode || req.user.location?.pincode,
      };
    }
    ...
    return sendError(res, { message: 'Failed to update profile' });
  }
};
