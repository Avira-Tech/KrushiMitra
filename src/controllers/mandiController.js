...
const AGMARKNET_API_KEY =
  process.env.AGMARKNET_API_KEY || '579b464db66ec23bdd000001cdd3946e44ce4aad7209ff7b23ac571b';
...
const fetchAndSyncFromAPI = async ({ commodity, state, district, limit = 100 } = {}) => {
  ...
  if (validRecords.length === 0 && records.length > 0) {
    logger.warn(
      `⚠️ Mandi: Received ${records.length} records but 0 passed validation. Possible schema mismatch at source.`,
    );
...
const getPrices = async (req, res) => {
  try {
    const { commodity, state = 'Gujarat', district, limit = 50, skip = 0, useCache } = req.query;
    ...
    if (forceLive) {
      logger.info(
        `🔄 Mandi: Triggering background sync for ${state}${commodity ? ' - ' + commodity : ''}`,
      );
      fetchAndSyncFromAPI({ commodity, state, district, limit: parseInt(limit) })
        .then((live) => {
          if (live.length > 0) {
            ...
            logger.info(
              `✅ Mandi: Background sync completed (${live.length} records). First crop: ${live[0].Commodity || live[0].commodity}`,
            );
          }
        })
        .catch((err) => {
    ...
    if (useCache !== 'true') {
      try {
        ...
        const liveData = await fetchAndSyncFromAPI({
          commodity,
          state,
          district,
          limit: parseInt(limit),
        ...
        if (validNormalized.length > 0) {
          lastSuccessfulFetch = Date.now();
          logger.info(
            `✨ Mandi: Live fetch successful, returning ${validNormalized.length} records.`,
          );
          return res.status(200).json({
            success: true,
          ...
          logger.warn(
            `⚠️ Mandi: Live fetch returned ${liveData.length} records but 0 passed final normalization for ${state}`,
          );
        }
      } catch (err) {
  ...
  try {
    ...
    if (!recommendation || recommendation.success === false) {
      return res.status(400).json({
        success: false,
        error: recommendation?.message || 'Unable to generate recommendation',
      });
    }
    ...
    return res
      .status(500)
      .json({ success: false, error: 'Failed to generate price recommendation' });
  }
};
